...
class RazorPayWalletScreen extends StatefulWidget {
  final WalletAddMoneyModel snapshotData;

  const RazorPayWalletScreen(
...
class _RazorPayScreenState extends State<RazorPayWalletScreen> {
  ...
  void openCheckout() async {
    var options = {
      ...
      //  'amount': 100,
      'name': 'Desichatkara',
      'description': 'Desichatkara Wallet Reload Payment',
      'order_id': widget.snapshotData.data.tansactionDetails.gatewayOrderId,
      'prefill': {
        'contact': '$userPhone',
        'email': '$userEmail',
        'name': '$userName'
  ...
  Future<void> _handlePaymentSuccess(PaymentSuccessResponse response) async {
    Map _body = {
      "transaction_id":"${widget.snapshotData.data.tansactionDetails.transactionId}",
      "gateway_order_id":"${widget.snapshotData.data.tansactionDetails.gatewayOrderId}",
      "transaction_status":"Success" // Failed if not Successful
    };
    _response = await _apiBaseHelper.postWithHeader(
        "wallet/status/update", _body, "Bearer $userToken");
