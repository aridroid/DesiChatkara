...
import 'bloc/mapTrackingBloc.dart';
import 'model/mapTrackingModel.dart';
import 'repository/mapTrackingRepository.dart';
import 'package:desichatkara/app_screens/orderDetails_screen/model/orderDetailsModel.dart' as ot;

class MapTrackingPage extends StatefulWidget {
  final ot.Data orderData;

  const MapTrackingPage({Key key, this.orderData}) : super(key: key);



 // MapTrackingPage(this.orderData, {String orderData});
...
class _MapTrackingPageState extends State<MapTrackingPage> {
  final ot.Data orderDetailsData;
  ...
  Future<void> createSharedPref() async {
    ...
    Map body = {"carrier_id": "${widget.orderData.carrierId}"};

    prefs = await SharedPreferences.getInstance();
    userToken = prefs.getString("user_token");
    _trackingStream = _mapTrackingBloc.mapTrackingStream;

    _mapTrackingBloc.mapTracking(orderDetailsData.carrierId, userToken);
    locUpdate = true;
  ...
  Future<Uint8List> getMarker() async {
    ByteData byteData = await DefaultAssetBundle.of(context).load("images/bike_icon.png");
